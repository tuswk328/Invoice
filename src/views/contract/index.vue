<template>
  <div class="app-container">
    <!--表单组件-->
    <eForm ref="form" :isAdd="isAdd" :carrierList="carrierList" />
    <el-row :gutter="24">
      <el-col>
        <!--工具栏-->
        <div class="head-container">
          ...
          <el-row>
            <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8" class="filter-item">
              ...
              <el-input v-model="query.contractNum" clearable placeholder="请输入合同号" style="width: 200px;"
            ...
            <el-col :xs="24" :sm="24" :md="12" :lg="8" :xl="8" class="filter-item">
              ...
              <el-select filterable v-model="query.carrier" clearable placeholder="请输入承运方" @keyup.enter.native="toQuery"
                style="width: 200px;">
                <el-option v-for="item in carrierList" :key="item.value" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
            </el-col>
          </el-row>
          ...
          <div style="display: inline-block;margin: 0px 2px;">
            <el-button v-permission="['ADMIN','CONTRACT_ALL','CONTRACT_CREATE']" class="filter-item" size="mini" type="primary"
              icon="el-icon-plus" @click="add">新增</el-button>
          </div>
          <!-- 作废 -->
          <div style="display: inline-block;margin: 0px 2px;">
            <el-button v-permission="['ADMIN','RENTCONTRACT_ALL','RENTCONTRACT_CANCEL']" class="filter-item" size="mini"
              type="danger" icon="el-icon-delete" @click="cancel">作废</el-button>
          </div>
          <!-- 导出 -->
        ...
        <!--分页组件-->
        <el-pagination :total="total" style="margin-top: 8px;" :current-page="page + 1" layout="total, prev, pager, next, sizes"
  ...
  export default {
    ...
    methods: {
      ...
      beforeInit() {
        ...
        //最高级别查询所有数据
        if (contractNum) {
          this.params['contractNum'] = contractNum
        }
        if (drawwe) {
      ...
      download() {
        if (this.vertifys == '') {
          ...
          import('@/utils/export2Excel').then(excel => {
            ...
            const filterVal = ['contractNo', 'contractStatusName', 'createDate', 'contractNum', 'drawwe',
              'carrierName', 'startDate', 'endDate', 'creatorName'
            ]
