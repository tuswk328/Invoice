  ...
  <el-dialog :visible.sync="dialog" :before-close="cancel" :title="isAdd ? '新增用户' : '编辑用户'" append-to-body width="570px">
    <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="66px">
      ...
      <el-form-item label="数据权限">
        <treeselect v-model="deptIds" multiple  :options="depts" style="width: 370px" placeholder="请选择" />
      </el-form-item>
    </el-form>
          ...
          this.$refs['form'].validate((valid) => {
            if (valid) {
              if (this.deptId === null || this.deptId === undefined) {
                ...
                this.deptIds.forEach(function(data, index) {
                  const dept = {
                    id: data
                  }
                  _this.form.deptsAuthority.push(dept)
                })
                if (this.isAdd) {
                  this.doAdd()
                } else this.doEdit()
